...
def read_dinamic_data(hoja, configuracion, table_name, sheet_name, create_table, filename):
	...
	# 		"__x_axis__": 20,
	# 		"__parser_config__": { "row_start": 21 ,"number_of_rows": 3, "include_condition": true },
	# 		"__static_columns__": [ { "column": "B", "name": "TYPE", "type": "VARCHAR" } ],
	# 		"__data_schema__": "dynamic"


    # "Occupancy_year_to_date": {
	# 		"__y_axis__": "V",
	# 		"__null_condition_row__": 19,
	# 		"__x_axis__": 20,
	# 		"__parser_config__": { "row_start": 21 ,"number_of_rows": 3, "include_condition": false },
	# 		"__static_columns__": [ { "column": "B", "name": "TYPE", "type": "VARCHAR" } ],
	# 		"__data_schema__": "dynamic"
	# 	},
    ...
    y_axis = configuracion["__y_axis__"]
    x_axis = configuracion["__x_axis__"]
    static_columns = configuracion["__static_columns__"]
    column_start = get_column_number(y_axis)
    year_start = get_value("name", "VARCHAR", 20 - 1, get_column_number("V") - 1, True)
    ...
    if "year" in table_name or "running" in table_name:
        query_creation_str = f'TYPE VARCHAR, "{year_start}" DOUBLE, "{year_start + 1}" DOUBLE, "{year_start + 2}" DOUBLE'

    elif "Month" in sheet_name:
        query_creation_str = "TYPE VARCHAR, "
        for m in range(31):
            if m == 30:
                query_creation_str += f'"{m+1}" DOUBLE'
            else:
                query_creation_str += f'"{m+1}" DOUBLE, '
    else:
        query_creation_str = "TYPE VARCHAR, YEAR INTEGER, Jan DOUBLE, Feb DOUBLE, Mar DOUBLE, Apr DOUBLE, May DOUBLE, Jun DOUBLE, Jul DOUBLE, Aug DOUBLE, Sep DOUBLE, Oct DOUBLE, Nov DOUBLE, Dec DOUBLE "
    ...
    for i in range(total_rows):
        ...
        x_term = get_value("name", "VARCHAR", x_axis - 1, column_start - 1, True) 
        ...
        while (isinstance(x_term, str) or (not pd.isnull(x_term) and not np.isnan(x_term))):
            ...
            #     if 'YEAR' in record:
            #         if first:
            #             query_creation_str += f' {x_term}_{record["YEAR"]} DOUBLE'
            #             first = False
            #         else: 
            #             query_creation_str += f', {x_term}_{record["YEAR"]} DOUBLE'
            #     else:
            #         if first:
            #             query_creation_str += f' YEAR_{x_term} DOUBLE'
            #             first = False
            #         else: 
            #             query_creation_str += f', YEAR_{x_term} DOUBLE'
...
# 					"outer_loop_index": "C"
# 				},
# 				"record_processor": "occunpancy_dow_processor"
# 			},
...
def read_full_dynamic_data(hoja, configuracion, table_name, sheet_name, create_table, filename):
    ...
    outer_loop_index = get_outer_loop_index(titles) # Positional index for the first record. C = 3
    inner_loop_index = get_inner_loop_reference(titles) # Positional index for the first title. 18
    total_records = count_records(titles) #18
    total_titles = count_titles(titles) #4

    query_creation_str = ""
    

    if "Week" in sheet_name:
        query_creation_str = f'TYPE VARCHAR, DAY_OF_WEEK VARCHAR, "CURRENT MONTH" DOUBLE, "YEAR TO DATE" DOUBLE, "RUNNING 3 MONTH" DOUBLE, "RUNNING 12 MONTH" DOUBLE'
    else:
        query_creation_str = "TYPE VARCHAR, YEAR INTEGER, Jan DOUBLE, Feb DOUBLE, Mar DOUBLE, Apr DOUBLE, May DOUBLE, Jun DOUBLE, Jul DOUBLE, Aug DOUBLE, Sep DOUBLE, Oct DOUBLE, Nov DOUBLE, Dec DOUBLE "
    ...
    if record_processor == "skip_line_processor":
        ...
        print("RECORD TYPE SUBTYPE CATEGORY PROCESSOR")
        ...
        print("RECORD STATIC CATEGORY PROCESSOR")
        ...
        print("OCCUPANCY DELTA DOW PROCESSOR")
        ...
        print("REVPAR DELTA DOW PROCESSOR")
        ...
        print("HORIZONTAL STATIC CATEGORY")
        ...
        print("HORIZONTAL STATIC SKIPLINE CATEGORY")
        ...
        print("HORIZONTAL CENSUS STATIC SKIPLINE CATEGORY")
        ...
        print("SEGMENTATION GLANCE STATIC CATEGORY REFERENCE PROCESSOR")
        ...
        print("SEGMENTATION GLANCE DELTA CATEGORY REFERENCE PROCESSOR")
        ...
        print("GLANCE STATIC CATEGORY REFERENCE PROCESSOR")
...
#     records = occunpancy_dow_processor(titles, inner_loop_index = 18, outer_loop_index = 3, total_titles = 4, total_records = 18, filename)
...
#     records = occunpancy_dow_processor(titles, inner_loop_index = 18, outer_loop_index = 3, total_titles = 4, total_records = 18, filename) typestr = My Property
...
def count_titles(titles):
    if titles["direction"] == "vertical":
        columm_check = get_column_number(titles["outer_loop_index"]) + 1 #4
...
def get_row_number(parser_config, row_start):
    if "number_of_rows" in parser_config and parser_config["number_of_rows"]:
        ...
        columm_check = get_column_number(parser_config["row_check"]) + 1
...
def read_range_data(hoja, configuracion, table_name, sheet_name, create_table, filename):
    ...
    for i in range(total_rows):
            ...
            if "__" not in variable:
                ...
                if 'value' in ubicacion:
                   valor = ubicacion['value'] 
                    ...
                    var_type = ubicacion['type']
                    columna_numero = get_column_number(column)
                    row = row_start + i
                    valor = get_value(variable, var_type, row - 1, columna_numero - 1, True)
        ...
        if sheet_name.upper() == "RESPONSE":
            record["PROPERTY_NAME"] = get_value("property_name", "VARCHAR", 1, 2)
            record["PROPERTY_ID"] = get_value("property_id", "VARCHAR", 2, 2)
            record["PERIOD"] = get_value("period", "VARCHAR", 3, 2)
...
def clean_value_output(value, value_type):
    print(value_type)
    print(value)
    if value_type in ['DOUBLE', 'INTEGER'] and (pd.isnull(value) or value == ""):
        return 0
    elif value_type not in ['DOUBLE', 'INTEGER']:
        if str(value).strip() == '':
            return 0
    elif value == 'Data is Insufficient':
        return 0
    return value
...
def insert_data(sheet_name, table_name, json_data, isMerge):
    ...
    try:
        # if json_data["PERIOD"]:
        #     json_data["PERIOD"] = datetime.strftime(json_data["PERIOD"], "%B %Y")
        ...
        print(f"The data for key 'PERIOD' not found in {json_data}")
...
def add_column(col, sheet_name, table_name):
    ...
    check_table_sql = f"""
        ...
        WHERE TABLE_SCHEMA = '{sheet_name.upper()}'
    ...
    if table_exists == 0:
        print(f"Table '{sheet_name}.{table_name}' does not exist.")
    else:
        check_column_sql = f"""
            SELECT COUNT(*) 
        ...
        if column_exists == 0:
            add_column_sql = f"""
                ALTER TABLE "{sheet_name.upper()}"."{table_name.upper()}"
...
for root, _, files in os.walk(current_dir):
    for file in files:
        if current_date in file:
            ...
            new_path = old_path
            new_filename = file
            if "Holiday_Inn_Express_&_Suites_Warwick_RI" not in file:
                new_filename = f"Holiday_Inn_Express_&_Suites_Warwick_RI_{file}"
...
# Print results
if _files:
    print(f"{len(_files)} files were found and renamed successfully:")
    for f in _files:
        print(f)
else:
...
crear_tablas = True
total = 0
yearUploaded = [2022, 2023]

for archivo_excel in _files:
    # Ruta al archivo de configuración JSON
    pattern = r"\d{2}\.(\d{4})"
    found = re.search(pattern, os.path.basename(archivo_excel))
    ...
    try:
        config = load_config(r'C:\Users\patel\Desktop\Work\innovo\STR-Anand' + "\\" + archivo_configuracion)
    except FileNotFoundError:
    ...
    for sheet_name, tablas in config.items():
        create_schemas(sheet_name)
        for table_name, sheet_config in tablas.items():

            if int(found.group(1)) not in yearUploaded:
                if "year" in table_name or "running" in table_name:
                    add_column(int(found.group(1)), sheet_name, table_name)
    ...
    if found.group(1) not in yearUploaded:
        yearUploaded.append(int(found.group(1)))
